...
VM_NO=$(virsh list --all --name | nl | xargs whiptail --menu "Select a VM to update" 0 0 0 3>&1 1>&2 2>&3)
...
- hosts: fedora
  ...
  - name: set hostname
    hostname:
      name: "$VM_NAME.localdomain"
  - name: update packages
    yum:
      name: "*"
      state: latest
  - name: set virt_use_nfs on in order to allow putting guest image on nfs
    seboolean:
      name: virt_use_nfs
      state: yes
      persistent: yes
